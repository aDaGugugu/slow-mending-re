...
public class com_set {
    private static int set(int num, Player player ,boolean hand){
        //获取物品
        ItemMeta itemMeta;
        ...
        //尝试更改
        try {
            itemMeta = MendingItem.setRemainderMendFrequency(itemMeta, Slow_mending_re.getMend_Frequency_Lore_Name(),num,Slow_mending_re.getOld_Mend_Frequency_Lore_Name());
            //修改物品
            if (hand){
            ...
            player.sendMessage("§b当前持有物品剩余修补次数已设置为："+num);
            return 0;
    ...
    public static void playerSet(Player player ,int num ,Player sender ,boolean hand){
        int result = set(num,player,hand);
        String whichhand;

        if (hand) whichhand = "主手";
        else whichhand = "副手";

        if (result == 0) sender.sendMessage("§b已将目标"+whichhand+"工具设置为："+num);
    ...
    public static void consSet(Player player ,int num ,boolean hand){
        ...
        if (result == 0) SLM.getLogger().info("§b已将目标"+whichhand+"工具设置为："+num);
